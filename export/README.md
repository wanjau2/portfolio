...
1. Replace the three placeholder rows in the Certifications table (`index.html`,
   search `TODO`) with real certificates, and add a `Verify` link where one exists.
...
4. Update `<link rel="canonical">` and the `og:url` / `url` values if the site
   lives somewhere other than https://wanjau.netlify.app/.
